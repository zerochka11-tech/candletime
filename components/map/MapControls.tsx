...
export function MapControls({ filters, onFiltersChange, loading }: MapControlsProps) {
  ...
  return (
    <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-lg shadow-md p-2.5 space-y-2 min-w-[160px] border border-slate-200/50 dark:border-slate-700/50">
      {/* Фильтр по типу */}
      <div>
        <select
          value={filters.type}
          onChange={(e) => onFiltersChange({ ...filters, type: e.target.value })}
          className="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-2 py-1.5 text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-500"
        >
          {types.map((type) => (
